...
export function NetworkGraph({ selectedId, onSelect, depth, pathNodes, pathMode, onPathSelect, edgeFilter, showDormant, privacy }) {
  ...
  const visEnt = useMemo(() => {
    if (depth === 1) return ENTITIES.filter((e) => directIds.has(e.id));
    return ENTITIES;
  }, [depth, directIds]);
  ...
  const node = (id) => ENTITIES.find((e) => e.id === id);
  const mx = Math.max(...RELATIONSHIPS.map((r) => r.volume));
    ...
    <div className="graph-container">
      <svg viewBox="0 0 800 560" className="w-full h-full">
        ...
        {visRel.map((rel, i) => {
          ...
          const op = isOnPath ? 1 : hNode ? (isConn ? 0.8 : 0.08) : (pathNodes && pathNodes.length > 0 ? 0.06 : (isDormant ? 0.15 : 0.35));
          ...
          const edgeColor = isOnPath ? QB.orange : isDormant ? QB.dormant : HOP_COLORS[edgeHopIdx];
          const markerEnd = isOnPath ? 'url(#arrow-path)' : isDormant ? 'url(#arrow-dormant)' : `url(#arrow-hop-${edgeHopIdx})`;
          ...
          return (
            <g key={'e' + i}>
              <line x1={x1} y1={y1} x2={x2} y2={y2}
                ...
                strokeDasharray={isDormant && !isOnPath ? '6 4' : 'none'}
                markerEnd={markerEnd}
                className="transition-all duration-300 cursor-pointer"
                onMouseEnter={() => setHEdge(i)} onMouseLeave={() => setHEdge(null)} />
              {hEdge === i && (
                <g>
                  <rect x={(s.x + t.x) / 2 - 50} y={(s.y + t.y) / 2 - 14} width="100" height="26" rx="4" fill="white" stroke="#D0D5DD" />
                  <text x={(s.x + t.x) / 2} y={(s.y + t.y) / 2 + 3} textAnchor="middle" fill={QB.textPrimary} fontSize="10" fontFamily="system-ui">
                    {getRelType(rel, 'e1') === 'vendor' ? '\u2190 Vendor' : '\u2192 Client'} &middot; {fmt(rel.volume)}/yr
                  </text>
                </g>
              )}
            </g>
