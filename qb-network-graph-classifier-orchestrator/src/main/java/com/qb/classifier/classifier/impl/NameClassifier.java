 ...
 */
public class NameClassifier implements Classifier<NameClassifier.Result> {
    ...
    private static final Result EMPTY = new Result("", "", List.of(), null);

    @Override
    public Result classify(EntityConnection input) {
        String displayName = input.displayName();
        if (displayName == null || displayName.isBlank()) return EMPTY;
        ...
        // 3. Strip trailing suffixes (loop to handle stacked suffixes)
        String prev;
        do {
            prev = upper;
